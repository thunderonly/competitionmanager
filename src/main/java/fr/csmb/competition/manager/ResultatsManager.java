...
public class ResultatsManager {
     ...
     * @param epreuveBean
     */
    private void createBodyForClassementParticipant(XSSFSheet sheet, int rowCount, int colCount, XSSFCellStyle styleData,
            CompetitionBean competitionBean, EpreuveBean epreuveBean) {

        SortedList<ParticipantBean> sortableList = new SortedList<ParticipantBean>(
                competitionBean.getParticipantByEpreuve(epreuveBean), new ComparatorParticipantCombat());
        int nbPart = 0;
    ...
    private void createClassementClub(XSSFSheet sheet, int rowCount, int colCount, XSSFCellStyle styleTitle2,
        ...
        Comparator comparator = new ComparatorClubTotalGeneral();
        if (typeEpreuve != null) {
            switch (typeEpreuve) {
                case TECHNIQUE:
